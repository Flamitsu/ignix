...
pub fn get_disk_partition_uevent(
    device_sysfs: &Path,
    device: &str,
...
pub fn get_esp_partition(
    ...
    for subdevice in subdevice_uevent {
        ...
        for line in archive.lines() {
            ...
            } else if let Some(value) = line.strip_prefix(LinuxTags::DEVTYPE)
                && value == "partition"
            {
                is_partition = true
            } else if let Some(value) = line.strip_prefix(LinuxTags::PARTUUID)
                && value == partition_guid
            {
                guid_matches = true;
            }
        }
