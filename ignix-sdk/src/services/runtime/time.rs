...
pub fn get_time() -> Result<TimeStruct, IgnixError> {
    /* SAFETY
    ...
    let status = unsafe { (get_runtime_services().get_time)(&mut time, &mut time_capabilities) };
    if status.is_error() {
...
/// The platform should describe this runtime service as unsupported at runtime via an
/// EFI_RT_PROPERTIES_TABLE configuration table.
pub fn set_time(time: &Time) -> Result<(), IgnixError> {
    let status = unsafe { (get_runtime_services().set_time)(time) };
    if status.is_error() {
...
/// The platform should describe this runtime service as unsupported at runtime via an
/// EFI_RT_PROPERTIES_TABLE configuration table.
pub fn get_wakeup_time() -> Result<WakeupTime, IgnixError> {
    let mut enabled: Boolean = Boolean(0);
    let mut pending: Boolean = Boolean(0);
    let mut time: Time = unsafe { zeroed() };
    let status =
        unsafe { (get_runtime_services().get_wakeup_time)(&mut enabled, &mut pending, &mut time) };
    if status.is_error() {
...
/// The platform should describe this runtime service as unsupported at runtime via an
/// EFI_RT_PROPERTIES_TABLE configuration table.
pub fn set_wakeup_time(enable: bool, time: Option<&Time>) -> Result<(), IgnixError> {
    assert!(
        enable && time.is_none(),
    ...
    let status = unsafe { (get_runtime_services().set_wakeup_time)(enable, time_ptr) };
    if status.is_error() {
