...
pub fn set_virtual_address_map() -> Result<(), IgnixError> {
    ...
    let status = unsafe {
        (get_runtime_services().set_virtual_address_map)(
            memory_map.map_size,
...
///
/// RETURN CODES:
/// EFI_NOT_FOUND The pointer pointed to by Address was not found to be part of the current memory map.
...
pub fn convert_pointer(
    ...
    let status = unsafe { (get_runtime_services().convert_pointer)(debug_disposition, addr_ptr) };
    Ok(())
}
