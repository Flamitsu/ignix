...
pub fn raise_tpl(new_tpl: Tpl) -> Option<TplGuardian> {
    Some(TplGuardian {
        old_tlp: unsafe { (get_boot_services().raise_tpl)(new_tpl) },
    })
}
...
pub(crate) fn restore_tpl(old_tpl: Tpl) {
    unsafe { (get_boot_services().restore_tpl)(old_tpl) }
}
/// Creates an event
/// RETURN CODES:
///
...
pub fn create_event<'a, T>(
    ...
    let status = unsafe {
        (get_boot_services().create_event)(
            event_type,
            tpl,
            notify_function,
            notify_context,
...
pub fn create_event_ex<'a, T>(
    ...
    let status = unsafe {
        (get_boot_services().create_event_ex)(
            event_type,
            tpl,
            notify_function,
            data_context,
...
/// function.
pub(crate) fn close_event(event: Event) -> Result<(), IgnixError> {
    let status = unsafe { (get_boot_services().close_event)(event) };
    if status.is_error() {
...
pub fn signal_event(event: &IgnixEvent) -> Result<(), IgnixError> {
    let status = unsafe { (get_boot_services().signal_event)(event.raw_event) };
    if status.is_error() {
...
pub fn wait_for_event<const N: usize>(event: &[Event]) -> Result<usize, IgnixError> {
    let mut index = 0;
    let status =
        unsafe { (get_boot_services().wait_for_event)(event.len(), event.as_ptr(), &mut index) };
    if status.is_success() {
        return Ok(index);
    }
