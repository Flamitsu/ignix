...
pub fn stall(duration: Duration) -> Result<(), IgnixError> {
    let microseconds = duration.as_micros() as usize;
    let status = unsafe { (get_boot_services().stall)(microseconds) };
...
pub fn get_next_monotonic_count(count: u64) -> Result<u64, IgnixError> {
    // I did this so the variable itself isn't modified, it's modified value its a
    // returned value.
    let mut shadow = count;
    let status = unsafe { (get_boot_services().get_next_monotonic_count)(&mut shadow) };
    if status.is_error() {
...
pub fn install_configuration_table<T: Table>(
    ...
    let status = unsafe { (get_boot_services().install_configuration_table)(guid, table_ptr) };
...
pub fn calculate_crc32<const N: usize>(buffer: [u8; N]) -> Result<u32, IgnixError> {
    let mut crc32: u32 = 0;
    let status = unsafe {
        (get_boot_services().calculate_crc32)(
