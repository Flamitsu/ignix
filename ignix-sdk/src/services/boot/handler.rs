...
use crate::{
    init::HANDLE, protocol::DevicePathProtocol, table::get_boot_services, types::{
        DevicePath, Event, FixedHandleList, Guid, Handle, HandleBuffer,
...
pub(crate) fn uninstall_protocol_interface(
    ...
    let status = unsafe {
        (get_boot_services().uninstall_protocol_interface)(handle, protocol, interface_ptr)
    };
    if status.is_error() {
...
pub fn locate_handle<const N: usize>(
    ...
    let status = unsafe {
        (get_boot_services().locate_handle)(
            search_type,
            protocol_ptr,
            search_key_ptr,
            &mut buffer_size,
...
pub fn handle_protocol<'p, 'i: 'p>(
    ...
    let status = unsafe {
        (get_boot_services().handle_protocol)(
            handle_ptr,
            protocol,
...
pub fn locate_device_path(
    ...
    let status = unsafe {
        (get_boot_services().locate_device_path)(
            protocol,
...
pub fn open_protocol_information<T>(
    ...
    let status = unsafe {
        (get_boot_services().open_protocol_information)(
            protocol.handle,
...
pub fn locate_handle_buffer(
    ...
    let status = unsafe {
        (get_boot_services().locate_handle_buffer)(
            search_type,
            protocol_ptr,
            search_key_ptr,
            &mut num_handles,
